...
if( isset($_GET['list_id']) && isset($_GET['item_id']) ){
    ...
    if( isset($_POST['edit_item']) ) {
        ...
        $sql = "SELECT lists.id as 'list_id', lists.name as 'list_name', items.description as 'item_text' FROM items INNER JOIN recambios ON items.list_id = lists.id WHERE items.id = $item_id LIMIT 1";
        $result = mysqli_query($db, $sql);
